...
class DeviceManager:
    ...
    def set_frame(self, parent):
        ...
        Device(frame, "Oscilloscope", INSTANCES.oscilloscope,
               VARIABLES.id_oscilloscope)
        Device(frame, "Monochromator", INSTANCES.monochromator,
               VARIABLES.id_monochromator)
        ...
        Device(frame, "Powermeter", INSTANCES.powermeter,
               VARIABLES.id_powermeter)
        Device(frame, "CWController", INSTANCES.cwcontroller,
               VARIABLES.id_cwcontroller)
        Device(frame, "Lockin (Top)", INSTANCES.lockin_top,
        ...
        ttk.Button(frame, text="Refresh Ports/Ids", command=self.refresh).pack(
            side="top", anchor="w", padx=(100, 0), pady=(0, 0))
        self.text_resources = tk.Text(frame)
        self.text_resources.pack(
            side="left", anchor="w", padx=(100, 0), pady=(0, 0))
        ...
        ttk.Label(frame, text="Notification User Key:").pack(
            side="top", anchor="w", padx=(100, 0), pady=(30, 0))
        ttk.Entry(frame, textvariable=VARIABLES.var_entry_notification_user_key,
                  width=50).pack(side="top", anchor="w", padx=(100, 0), pady=(0, 0))
        from utils.config import UTILS
        ttk.Button(frame, text="Test Notification", command=lambda: UTILS.push_notification(
            "Hello World!")).pack(side="top", anchor="w", padx=(100, 0), pady=(30, 0))
        return frame

    def get_resources(self):
        try:
            # ["abc", "def", "34dfgdgs"] #
            resource_list = ResourceManager().list_resources()
...
class Device(ttk.Frame):
    def __init__(self, parent, name, instance, id_string_var):
        ...
        if self.id_string_var:
            ttk.Entry(self, textvariable=self.id_string_var).pack(
                side="left", padx=10)
        else:
            ttk.Label(self, text='N/A',
